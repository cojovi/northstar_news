#!/usr/bin/env python3
"""
Northstar Ledger Article Creator (Quick Version)
Fully autonomous blog post generator with integrated image generation.
Uses a fixed tone: professional, like a journalist, but still witty and fun. tone should reflect we lean republican
"""
...
# Fixed tone for quick article creation
FIXED_TONE = "professional, like a journalist, but still witty and fun. tone should reflect we lean republican"
...
def main():
    """Main execution flow."""
    print("=" * 60)
    print("🌟 NORTHSTAR LEDGER - QUICK ARTICLE CREATOR 🌟")
    print("=" * 60)
    print()
    ...
    # Step 2: Use fixed tone (no user prompt)
    custom_tone = FIXED_TONE
    print(f"✓ Using fixed tone: {custom_tone}")
    print()

    # Step 3: Detect input type and generate article
    input_type = detect_input_type(user_input)
    print(f"📋 Input type detected: {input_type.upper()}")
    print()

    # Step 3a: Perform web research if Tavily API key is available
    research_info = None
    if tavily_api_key:
    ...
    # Step 3b: Generate article with research context
    # Use empty string for tone since we're always using custom_tone
    default_tone = ""
    if input_type == 'url':
    ...
    # Step 5: Generate author and slug
    ...
    # Step 8: Commit and push to GitHub (optional, controlled by env var)
