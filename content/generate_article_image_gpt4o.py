#!/usr/bin/env python3
"""
Article Image Generator (GPT-4o Version)
Generates hero images for markdown articles using GPT-4o image generation.

Note: This script attempts to use GPT-4o for image generation. If this model
is not available or doesn't support image generation in your OpenAI account,
please use the standard generate_article_image.py (DALL-E 3) instead.
"""
...
def generate_image_prompt(client, article_info):
    ...
    print("Generating image prompt with GPT-4o...")
    
    prompt_text = f"""Based on this article, create a detailed image generation prompt.
...
def generate_image(client, prompt, output_path):
    """Generate an image using GPT-4o and save it."""
    print(f"Generating image with GPT-4o...")
    
    try:
        # Attempt to use GPT-4o for image generation
        # Note: If this model doesn't support image generation, this will fail
        response = client.images.generate(
            model="gpt-4o",  # Using GPT-4o instead of dall-e-3
            prompt=prompt,
        ...
        print(f"\n⚠️  Error generating image with GPT-4o: {str(e)}")
        print("\nThis could mean:")
        print("1. GPT-4o image generation is not available in your account")
        print("2. The model name is incorrect")
        print("3. This feature requires a different API endpoint")
        print("\n💡 Suggestion: Use the standard generate_article_image.py (DALL-E 3) instead")
        raise
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate hero images for markdown articles using GPT-4o'
    )
    parser.add_argument(
        'markdown_file',
        type=str,
    ...
    print("Using GPT-4o for image generation\n")
    with open(markdown_path, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    try:
        ...
        print(f"\nFailed to generate image. Exiting.")
        sys.exit(1)
